...
void *thread_routine(void *data)
{
	...
	// mutex.
		printf("%s\tAVANTThread [%ld]: compte au depart = %u.%s\n", YELLOW, tid, counter->count, NC);
	pthread_mutex_lock(&counter->count_mutex);
		usleep(1000000);
		printf("%sThread [%ld]: compte au depart = %u.%s\n", YELLOW, tid, counter->count, NC);
		usleep(1000000);
	pthread_mutex_unlock(&counter->count_mutex);
		printf("%s\tAPRES Thread [%ld]: compte au depart = %u.%s\n", YELLOW, tid, counter->count, NC);
	i = 0;
...
/*
question 1 :
		Pourquoi les mutex permettent davoir le bon resultat
		a tous les coups ?
		Quelle est la difference entre avec et sans ?
 */